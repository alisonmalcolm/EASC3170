...
def plotProfile(prob2D, x0, data, Binc, Bdec, Bigrf, susc, Q, rinc, rdec):
    ...
    xlim = [0,distance[-1]]
    prob1D = simpegPF.problem()
    srvy1D = simpegPF.survey()
...
def plotObj3D(p, rx_h, elev, azim, npts2D, xylim,
              profile=None, x0=15., y0=0., fig=None, axs=None, plotSurvey=True,cstring='w'):
...
#    print xyz
    ...
    axs.add_collection3d(Poly3DCollection([zip(xyz[:4, 0]+p.xc,
                                               xyz[:4, 1]+p.yc,
                                               xyz[:4, 2]+p.zc)], facecolors=cstring))
 #   print zip(xyz[:4, 0]+p.xc,xyz[:4, 1]+p.yc,xyz[:4, 2]+p.zc)
...
def plogMagSurvey2D(prob2D, susc, Einc, Edec, Bigrf, comp, irt,  Q, rinc, rdec, fig=None, axs1=None, axs2=None):
    ...
    # The simpegPF problem created is stored in result[1]
    # prob2D = Box.result[1]
...
def ViewPrismAndData():
    ...
    Q = widgets.interactive(PrismAndData \
                            ...
                            , npts2D=widgets.FloatSlider(min=5, max=100, step=5, value=20, continuous_update=False) \
...
def ViewPipeByPrism(depth, dec, xylim=3.):
    ...
    Q = widgets.interactive(PipeByPrism \
                            ...
                            , pdec=widgets.FloatSlider(min=-90., max=90., step=5.,value=0.0) \
                            , x0=widgets.FloatSlider(min=-5., max=5., step=0.1,value=0.0) \
...
def ViewPipeIncField(Box):
    ...
    def SetupPipeField(Binc,Bdec,BF,susc,rx_h,depth,x0,profile):
