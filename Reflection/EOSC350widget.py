...
def NMOstack(data, xorig, time, v,t0):
    ...
    shiftdat=np.zeros_like(data)
    for i in range(xorig.size):
        shiftdat[i]=np.roll(data[i],-indmin[i])
        singletrace+=shiftdat[i]
...
def NoisyNMOWidget(t0, v):
    ...
    singletrace = NMOstack(syndata, xorig, time_data, v,t0) 
...
def NMOstackthree(data, tintercept, v1, v2, v3):
    ...
    for itry in range(3):
        traces[itry,:] = NMOstack(data, xorig, time, vtemp[itry],tintercept)
...
#        toffset = np.sqrt(xorig**2/v[i]**2+t0**2)
#        Time = (time.reshape([1,-1])).repeat(data.shape[0], axis=0)
#        Toffset = (toffset.reshape([-1,1])).repeat(data.shape[1], axis=1)
#        print(np.shape(Toffset))
#        print(np.min(Toffset))
#        print(np.max(Toffset))
#        indmin = np.argmin(abs(Time[i]-Toffset), axis=1)
...
def NMOstackSingle(data, tintercept, v):
    ...
    xorig = np.arange(38)*dx
    time = np.load('time1.npy')
    singletrace = NMOstack(data, xorig, time, v,tintercept)
